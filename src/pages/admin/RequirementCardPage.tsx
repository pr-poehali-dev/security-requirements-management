...
function renderMarkdown(text: string) {
  return text
    ...
    .replace(/^### (.+)$/gm, '<h3 class="text-sm font-semibold mt-2 mb-1 text-muted-foreground">$1</h3>')
...
export function RequirementCardPage() {
  ...
  const { requirements, addRequirement, updateRequirement, deleteRequirement, currentUser } = useStore();
  ...
  const handleSave = () => {
    if (isNew) {
      const created = addRequirement({ name, owner, status, description });
      navigate(`/admin/requirements/${created.id}`, { replace: true });
    } else {
      updateRequirement(id!, { name, owner, status, description });
      setEditing(false);
    }
  };
  ...
  const item = isNew ? null : requirements.find(r => r.id === id)!;

  return (
    <div className="p-6 max-w-3xl mx-auto animate-fade-in">
      <div className="flex items-center gap-1.5 text-xs text-muted-foreground mb-5">
        <Link to={`${isAdmin ? '/admin' : '/library'}/requirements`} className="hover:text-foreground transition-colors">Требования</Link>
        <Icon name="ChevronRight" size={12} />
        <span className="text-foreground">{isNew ? 'Новое требование' : item?.name}</span>
      </div>
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-start justify-between">
          <div>
            ...
            <h1 className="text-lg font-semibold">{isNew ? 'Новое требование' : item?.name}</h1>
            {item && (
          ...
          {(isNew || activeTab === 'info') && (
            <div className="space-y-4">
              ...
              <div>
                ...
                {editing ? (
                  <Textarea className="mt-1 min-h-[200px] font-mono-ibm text-xs" value={description} onChange={e => setDescription(e.target.value)} placeholder="## Заголовок&#10;&#10;Описание требования..." />
                ) : (
